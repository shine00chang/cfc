...
<main class="flex flex-row items-center h-full">
  <div class="grid grid-cols-1 lg:grid-cols-2 place-items-center h-[70%]">
    ...
    <div class="flex flex-col items-center">
      ...
      <div class="join">
        <button on:click={onCreate} class="btn btn-accent join-item w-24">create</button>
        <button on:click={onLogin} class="btn btn-accent join-item w-24">login</button>
      </div>
    </div>
  </div>
</main>

<div role="alert" class="absolute top-5 alert alert-success {create_success?"":"hidden"}">
  <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
  <span>Creation Successful</span>
</div>

<div role="alert" class="absolute top-5 alert alert-error {error_message?"":"hidden"}">
